import chroma from 'chroma-js'
import { memo } from 'react'

// types and interfaces
import type { GlowTopProps } from './types'

const GlowTop = ({ color = '#1fb76b', opacity = 0.35, zIndex = 1 }: GlowTopProps) => {
  const glowRgba = chroma(color).alpha(opacity).css()

  return (
    <div
      className="pointer-events-none"
      data-testid="glow-top__container"
      style={{
        background: `radial-gradient(48.78087172574147% 48.780871725741484% at 52.26579732061952% 52.311511675873824%, ${glowRgba}, rgba(0,0,0,0))`,
        flex: 'none',
        height: '726px',
        left: 'calc(50.00000000000002% - 345px / 2)',
        position: 'absolute',
        top: '-328px',
        userSelect: 'none',
        width: '345px',
        zIndex,
        animation: 'glow-drift-top 22s ease-in-out infinite',
        willChange: 'transform, opacity'
      }}
    />
  )
}

export default memo(GlowTop)
